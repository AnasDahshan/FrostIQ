...
void PrintRxPacket(Ptr<const Packet> packet, const Address &address)
{
  NS_LOG_INFO("Received packet: " << packet->GetSize() << " bytes from " << address);
}
...
int main(int argc, char *argv[])
{
  ...
  devices1 = pointToPoint.Install(nodes.Get(0), nodes.Get(1)); // Link between node 0 and node 1
  devices2 = pointToPoint2.Install(nodes.Get(1), nodes.Get(2)); // Link between node 1 and node 2
  ...
  address2.SetBase("9.1.1.0", "255.255.255.0");
